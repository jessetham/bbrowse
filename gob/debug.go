...
func (deb *debugger) dump(format string, args ...any) {
	...
	fmt.Fprintf(deb.w, format+" ", args...)
	if !deb.remainingKnown {
		return
	}
	if deb.remain < 0 {
		fmt.Fprintf(deb.w, "remaining byte count is negative! %d\n", deb.remain)
		return
	}
...
func (deb *debugger) nilInterfaceValue(indent tab) int {
	fmt.Fprintf(deb.w, "%snil interface\n", indent)
	return 0
}
...
func (deb *debugger) nonNilInterfaceValue(indent tab, nameLen int) {
	...
	for {
		...
		if id < 0 {
			...
			fmt.Fprintf(deb.w, "%sinterface value, type %q id=%d; valueLength %d\n", indent, name, id, x)
			deb.value(indent, id)
			break
		}
	}
...
func (deb *debugger) printWireType(indent tab, wire *wireType) {
	...
	case wire.StructT != nil:
		...
		// 	fmt.Fprintf(deb.w, "%sfield %d:\t%s\tid=%d\n", indent+1, i, field.Name, field.Id)
		// }
...
func (deb *debugger) printBuiltin(indent tab, id typeId) {
	...
	case tBool:
		x := deb.int64()
		if x == 0 {
			fmt.Fprintf(deb.w, "%sfalse\n", indent)
		} else {
			fmt.Fprintf(deb.w, "%strue\n", indent)
		}
	case tInt:
		x := deb.int64()
		fmt.Fprintf(deb.w, "%s%d\n", indent, x)
	case tUint:
		x := deb.uint64()
		fmt.Fprintf(deb.w, "%s%d\n", indent, x)
	case tFloat:
		x := deb.uint64()
		fmt.Fprintf(deb.w, "%s%g\n", indent, float64FromBits(x))
	case tComplex:
		r := deb.uint64()
		i := deb.uint64()
		fmt.Fprintf(deb.w, "%s%g+%gi\n", indent, float64FromBits(r), float64FromBits(i))
	case tBytes:
		x := int(deb.uint64())
		b := make([]byte, x)
		deb.r.Read(b)
		deb.consumed(x)
		fmt.Fprintf(deb.w, "%s{% x}=%q\n", indent, b, b)
	case tString:
		x := int(deb.uint64())
		b := make([]byte, x)
		deb.r.Read(b)
		deb.consumed(x)
		fmt.Fprintf(deb.w, "%s%q\n", indent, b)
	default:
...
func (deb *debugger) arrayValue(indent tab, wire *wireType) {
	...
	if length != wire.ArrayT.Len {
		fmt.Fprintf(deb.w, "%s(wrong length for array: %d should be %d)\n", indent, length, wire.ArrayT.Len)
	}
}
...
func (deb *debugger) gobEncoderValue(indent tab, id typeId) {
	...
	fmt.Fprintf(deb.w, "%s%s (implements GobEncoder)\n", indent, id.name())
	data := make([]byte, len)
	_, err := deb.r.Read(data)
	if err != nil {
		errorf("gobEncoder data read: %s", err)
	}
	fmt.Fprintf(deb.w, "%s[% .2x]\n", indent+1, data)
}
